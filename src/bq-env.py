...
def load_credentials():
    # First, check if JSON credentials are directly in the environment
    creds_json = os.getenv("BQ_CREDENTIALS")
    if creds_json:
        try:
            info = json.loads(creds_json)
            return Credentials.from_service_account_info(info)
        except json.JSONDecodeError:
            raise ValueError("Invalid JSON in BQ_CREDENTIALS environment variable")

    # Otherwise, fall back to a file path
